...
func (b *Ball) Move(g *Game) {
	...
	b.PlayerCollision(g.Player1.Pos.X + g.Player1.Size.X, b.Pos.X, &g.Player1)
	b.PlayerCollision(g.Player2.Pos.X, b.Pos.X + b.Size.X, &g.Player2)
	
	b.Pos.X += b.Velocity.X
	b.Pos.Y += b.Velocity.Y
}

func (b *Ball) PlayerCollision(xBoundPlayer float32, xBoundBall float32, p *Player) {
	if xBoundBall >= xBoundPlayer && xBoundBall <= xBoundPlayer &&
		b.Pos.Y >= p.Pos.Y && b.Pos.Y + b.Size.Y <= p.Pos.Y + p.Size.Y {
		b.Velocity.X*=-1
		b.NumHits+=0.2
...
func (g *Game) StateUpdate(screen *ebiten.Image) {
	...
	if g.isGoal {
		...
		rect := ebiten.NewImage(int(g.GameScreen.Size.X * 0.70), int(g.GameScreen.Size.Y * 0.70))
		vector.DrawFilledRect(rect, 0, 0, g.GameScreen.Size.X * 0.70, g.GameScreen.Size.Y * 0.70, color.White, true)

		drawOptions.GeoM.Scale(1.0, 1.0)
		drawOptions.GeoM.Translate(float64(g.GameScreen.Size.X / 2) - 200, float64(g.GameScreen.Size.Y/2) - 100)
		screen.DrawImage(rect, drawOptions)

		msg := "Goal!\nScore is:\n1 - 0\n Tap Enter to continue..."
    	text.Draw(screen, msg, basicfont.Face7x13, 60, 90, color.RGBA{R: 255, G: 0, B: 0, A: 255})
	}
}
