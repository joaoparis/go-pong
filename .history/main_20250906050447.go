...
func (g *Game) isPaused() {
	if ebiten.IsKeyPressed(ebiten.KeySpace) && !game.isGoal {
...
func (g *Game) StateUpdate(screen *ebiten.Image) {
	...
	if g.isGoal {
		...
		DrawText(screen, "GOAL!", g.GameScreen)
		DrawText(screen, "GOAL!", g.GameScreen)
		DrawText(screen, "GOAL!", g.GameScreen)
	} else if g.isPaused {
...
func DrawText(screen *ebiten.Image, text string, size *GameScreen) void {
		face := text.NewGoXFace(basicfont.Face7x13)
        opts := &text.DrawOptions{}
