...
func (game *Game) Update() error {
	if ebiten.IsKeyPressed(ebiten.KeyR) { game.start() }
	if game.isGamePaused() { return nil }
	if game.isGoal { return nil }
...
func (game *Game) isGamePaused() bool {
	if ebiten.IsKeyPressed(ebiten.KeyEscape) && !game.isGoal {
        // Only toggle once per press to avoid rapid switching
...
func (g *Game) start() {
	...
	ball := &Ball{
		Pos: Double{
			X: g.Window.X/2 - float32(ballSizeX)/2,
			Y: g.Window.Y/2 - float32(ballSizeY)/2,
		},
		Size: Double{
			X: float32ballSizeX,
			Y: float32ballSizeY,
	...
	player1 := &Player{
		Pos: Double{
			...
			Y: g.Window.Y/2 - float32(playerSizeY)/2,
		},
		Size: Double{
	...
	player2 := &Player{
		Pos: Double{
			X: g.Window.X - playerSizeX - playerBoundGap,
			Y: g.Window.Y/2 - playerSizeY/2,
		},
		Size: Double{
			X: float32(playerSizeX),
			Y: float32(playerSizeY),
