...
func (game *Game) isGamePaused() bool {
	if ebiten.IsKeyPressed(ebiten.KeyEsc) && !game.isGoal {
        // Only toggle once per press to avoid rapid switching
        if inpututil.IsKeyJustPressed(ebiten.KeyEsc) {
...
func (g *Game) StateUpdate(screen *ebiten.Image) {
	...
	if g.isGoal {
        ...
        opts.GeoM.Translate(float64(g.GameScreen.Size.X/2), float64(g.GameScreen.Size.Y/2))
...
func DrawText(screen *ebiten.Image, msg string, size Double) {
		face := text.NewGoXFace(basicfont.Face7x13)
        opts := &text.DrawOptions{}
  		opts.GeoM.Translate(float64(size.X/4), float64(size.Y/4))
