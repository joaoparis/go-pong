...
func (b *Ball) Move(g *Game) {
	...
	BallPlayerCollision(b, &g.Player1)
	PlayerCollision(b, &g.Player2)
	
	b.Pos.X += b.Velocity.X
	b.Pos.Y += b.Velocity.Y
}
func (b *Ball, p *Player) BallPlayerCollision() {
...
func (g *Game) StateUpdate(screen *ebiten.Image) {
	...
	if g.isGoal {
		...
		vector.DrawFilledRect(rect, 0, 0, 400, 200, color.RGBA{R: 0, G: 0, B: 255, A: 255}, true)

		drawOptions.GeoM.Scale(1.0, 1.0)
		drawOptions.GeoM.Translate(float64(g.GameScreen.Size.X / 2), float64(g.GameScreen.Size.Y/2))
		screen.DrawImage(rect, drawOptions)
	}
